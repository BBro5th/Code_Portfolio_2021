...
vec3 light_position_world  = vec3 (1.0, 1.5, 0.0);
...
vec3 colw = vec3 (0.8, 0.9, 0.1);
vec3 colc = vec3 (0.1, 0.1, 0.9);
float vecAng, r,g,b, wk;


// stuff for spotlight
vec3 lightDir = vec3(-2.0,-2.0,0.0);
...
void main () {
	...
	// get Angle vecAng
	vecAng = (dot(lightDir,normal_eye))/ sqrt(lightDir[0]*lightDir[0] + lightDir[1]*lightDir[1] + lightDir[2]*lightDir[2]);
	...
	// final colour
		//fragment_colour = vec4(Is/(dist*dist) + Id/(dist*dist) + Ia, 1.0); // (1.0, 0.5, 0.0, 1.0); //
	r = wk*(colw[0]) + (1-wk)*colc[0] ;
	g = wk*(colw[1]) + (1-wk)*colc[1] ;
	b = wk*(colw[2]) + (1-wk)*colc[2] ;
